...
class BlackTagPose(Node):
    def __init__(self):
        ...
        self.declare_parameter('world_frame', 'peg_board')
        ...
        self.tfbr = TransformBroadcaster(self)
        self.declare_parameter('save_log_images', False)
        self.declare_parameter('log_image_count', 5)
        self.save_log_images = bool(self.get_parameter('save_log_images').value)
        self.log_image_count = int(self.get_parameter('log_image_count').value)
        self.logged_images = 0
        
        self.ratio = 10 ##parameter to hceck the aspect ratio of a detected contour
        # Ensure directory exists if we plan to log
        if self.save_log_images:
            Path(self.debug_dir).mkdir(parents=True, exist_ok=True)
            self.get_logger().info(f"Image logging ENABLED: saving up to {self.log_image_count} frames to {self.debug_dir}")
    ...
    def _publish_world_pose(self, msg_stamp, R_cam_tag, t_cam_tag):
        ...
        try:
            ...
            self.get_logger().warn(f'No TF {self.world_frame}->{cam_frame}: {e}')
            return
        ...
        ps.header.frame_id = self.world_frame
        self.get_logger().info(f"tag in {self.world_frame}: ...")
    ...
    def _image_cb(self, msg: Image):
        ...
        if not self.have_caminfo:
            self.get_logger().info("No Cam Info Recieved")
            # wait for real intrinsics; avoids bad depth scale
            return
        ##IMAGE PREPROCESSING
        img = self.bridge.imgmsg_to_cv2(msg, desired_encoding='bgr8')
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        if self.save_log_images and self.logged_images < self.log_image_count:
            raw_path = os.path.join(self.debug_dir, f"frame_{self.logged_images:03d}_raw.png")
        ...
        ###IMAGE PREPORCESSING END ##
        # LOGGING ##
        if self.save_log_images and self.logged_images < self.log_image_count:
            bw_path = os.path.join(self.debug_dir, f"frame_{self.logged_images:03d}_bw.png")
        ...
        MIN_AREA = 50.0  # set to 0.0 to disable area filtering
        ...
        for i, c in enumerate(cnts):
            ...
            if verts != 4:
                reason = f"reject: verts={verts}"
            elif not is_convex:
                reason = "reject: non-convex"
            else:
                rect = cv2.minAreaRect(approx)
                (w, h) = rect[1]
                if min(w, h) <= 0:
                    reason = "reject: degenerate w/h"
                else:
                    ratio = max(w, h) / max(1e-6, min(w, h))
                    if MIN_AREA > 0.0 and area < MIN_AREA:
                        reason = f"reject: area={area:.1f} < {MIN_AREA}"
                    elif ratio >= self.ratio:
                        reason = f"reject: ratio={ratio:.2f} >= 1.3"
                    else:
                        reason = "ACCEPT"
                        accepted += 1
                        if area > best_area:
            ...
            # Print one concise line per contour
            self.get_logger().info(
                f"[c{i}] peri={peri:.1f} verts={verts} convex={is_convex} "
                f"area={area:.1f} w={w:.1f} h={h:.1f} ratio={ratio:.2f} -> {reason}"
            )

        # Final summary
        self.get_logger().info(
            f"[summary] total={len(cnts)} accepted={accepted} best_area={best_area:.1f}"
        )
        ...
        if not ok:
            return
        self.get_logger().info(f"solvePnP tvec (cam frame, m) = {tvec.ravel()}")

        R, _ = cv2.Rodrigues(rvec)
        qx, qy, qz, qw = rotmat_to_quaternion(R)
        if best is None:
            # Count this as a logged frame even if no quad found
            if self.save_log_images and self.logged_images < self.log_image_count:
        ...
        # Draw the detected quad for logging
        if self.save_log_images and self.logged_images < self.log_image_count:
            annotated = img.copy()
            # draw quad
            for i in range(4):
                p1 = tuple(int(v) for v in img_pts[i])
                p2 = tuple(int(v) for v in img_pts[(i+1) % 4])
                cv2.line(annotated, p1, p2, (0, 255, 0), 2)
            ann_path = os.path.join(self.debug_dir, f"frame_{self.logged_images:03d}_annotated.png")
