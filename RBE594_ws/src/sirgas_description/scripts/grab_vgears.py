...
# --- NEW IMPORTS FOR DYNAMIC DETACHMENT ---
# NOTE: Replace 'custom_link_service' with the actual package providing the service.
# For example, from 'linkattacher_msgs.srv import DetachLink'
try:
    from custom_link_service.srv import DetachJoint # Placeholder
except ImportError:
    # Fallback/Mockup if the service package is not yet installed
    class DetachJoint:
        class Request:
            parent_link_name = ''
            child_link_name = ''
...
# --- JOINT NAMES FOR DETACHMENT (Must match pba2.urdf.xacro link names) ---
PEG_BOARD_LINK = 'peg_board' # The parent link of the joint to break
FIRST_GEAR_LINK = 'first_gear' # The child link of the joint to break
DETACH_SERVICE_NAME = '/detach_joint' # Service name defined in panda_pba.urdf.xacro
# -----------------------------
...
class MoveItPanda(Node):
    def __init__(self):
        ...
        # --- NEW: Service client for Gazebo Joint Manipulation ---
        self.detach_client = self.create_client(
            DetachJoint,
            DETACH_SERVICE_NAME
        )
        self.get_logger().info(f"Waiting for Gazebo Detach Service: {DETACH_SERVICE_NAME}...")
        if not self.detach_client.wait_for_service(timeout_sec=10.0):
            self.get_logger().error("Timeout waiting for Detach Service! Cannot perform dynamic joint detachment.")
        else:
            self.get_logger().info("Detach Service found.")
        # --------------------------------------------------------
    ...
    def call_gazebo_detach_service(self):
        """
        Calls the Gazebo service to dynamically destroy the joint 
        between the peg board and the gear.
        """
        if not self.detach_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().error("Gazebo Detach Service is unavailable. Gear WILL NOT be able to move.")
            return False

        self.get_logger().info(f"Calling Gazebo service to detach joint between {PEG_BOARD_LINK} and {FIRST_GEAR_LINK}...")

        detach_req = DetachJoint.Request()
        # The joint to be broken is defined by the two links it connects
        detach_req.parent_link_name = PEG_BOARD_LINK # The fixed link 'peg_board'
        detach_req.child_link_name = FIRST_GEAR_LINK # The gear link 'first_gear'
        ...
        if result is not None and hasattr(result, 'success') and result.success:
            self.get_logger().info("SUCCESS: Joint dynamically destroyed. Gear is now free-floating.")
            return True
        else:
            self.get_logger().error(f"FAILED: Joint detachment failed. Message: {result.message if result and hasattr(result, 'message') else 'No result'}")
            return False
    ...
    def execute_complete_sequence(self):
        ...
        if self.move_gripper(self.gripper_positions['grasp']):
            self.get_logger().info("SUCCESS: Gripper closed (or gear grasped)! Proceeding to attach and detach.")
            
            # 5A: Attach gear to the hand (MoveIt Kinematic Attachment)
            self.attach_gear_to_hand()
            time.sleep(1.0)

            # 5B: Detach the physical joint in Gazebo (Physics Detachment)
            if not self.call_gazebo_detach_service():
                 self.get_logger().error("CRITICAL FAILED: Gazebo joint detachment failed. Gear will remain constrained!")
                 # Continue to cleanup, but sequence is marked as failed.
                 self.clear_gear_references() 
                 return False

            # 5C: Explicitly remove the original world copy to avoid CheckStartStateCollision
        ...
        if self.move_to_pose(lift_pose):
            self.get_logger().info("SUCCESS: Lift complete! Gear should be moving with the hand.")
        else:
